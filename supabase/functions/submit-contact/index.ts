...
function validateContactInput(data: any): { valid: boolean; error?: string } {
  ...
  if (!emailRegex.test(data.email.trim()) || data.email.length > 255) {
...
function validateEnrollmentInput(data: any): { valid: boolean; error?: string } {
  ...
  if (data.fullName.trim().length < 3 || data.fullName.trim().length > 100) {
  ...
  if (data.whatsapp.trim().length < 10 || data.whatsapp.trim().length > 20) {
  ...
  if (!data.courseFor || !["adult", "child"].includes(data.courseFor)) {
    return { valid: false, error: "Selecione para quem é o curso" };
  }
  if (data.courseFor === "child") {
    if (data.childAge === undefined || data.childAge === null) {
      return { valid: false, error: "Idade da criança é obrigatória" };
    }
    const age = Number(data.childAge);
    if (isNaN(age) || age < 3 || age > 17) {
      return { valid: false, error: "Idade deve estar entre 3 e 17 anos" };
    }
  }
  return { valid: true };
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Trigger email notification (non-blocking, don't wait for result)
